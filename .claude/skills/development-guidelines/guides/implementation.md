...
// ❌ Bad example: over-relying on type inference
...
```typescript
// Interfaces: extensible object types
interface Task {
  id: string;
  title: string;
  completed: boolean;
...
// Type aliases: union types, primitive types, etc.
...
// Boolean: start with is, has, should, can
const isValid = true;
...
```typescript
// ✅ Good example: appropriate error handling
async function getTask(id: string): Promise<Task> {
  try {
    ...
    throw new DatabaseError('Failed to fetch the task', error as Error);
  }
}
...
**Error messages**:
```typescript
// ✅ Good example: specific and indicates a solution
throw new ValidationError(
  'The title must be 1-200 characters. Current length: 250',
  'title',
  title
);
...
// Compute the maximum subarray sum with Kadane's algorithm
// Time complexity: O(n)
let maxSoFar = arr[0];
...
// HACK: Temporary workaround, refactoring needed later
```
...
// ❌ Bad example: O(n) search in an array
...
// ❌ Bad example: recomputing length every iteration
...
Verify before completing the implementation:
...
- [ ] TODOs and FIXMEs are recorded (if applicable)
