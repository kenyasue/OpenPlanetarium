...
void main() {
  ...
  group('ViewportController.pan', () {
    test(
      'center coordinates stay within the normalized range after panning',
      () {
        final container = _container();
        final controller = container.read(viewportControllerProvider.notifier);
    ...
    test(
      'dragging right moves stars right (the center moves toward the left sky)',
      () {
        final container = _container();
        final controller = container.read(viewportControllerProvider.notifier);
  ...
  group('ViewportController.centerOn / resize', () {
    ...
    test(
      'resize with the same or empty size does not advance the generation',
      () {
        final container = _container();
        final controller = container.read(viewportControllerProvider.notifier);
  ...
  group('Alt/Az lock on time change', () {
    ...
    test(
      'advancing time keeps the view center altitude/azimuth (RA/Dec changes)',
      () {
        final container = _container();
        final before = container.read(viewportControllerProvider);
    ...
    test(
      'Alt/Az does not drift across many time changes (playback equivalent)',
      () {
        final container = _container();
        final before = container.read(viewportControllerProvider);
        final t0 = container.read(timeControllerProvider);
  ...
  group('viewportStateProvider', () {
    ...
    test(
      'continues with the default (Tokyo) when location acquisition fails',
      () async {
        final container = _container();
