...
void main() {
  ...
  group('visibleDsosProvider', () {
    ...
    test(
      'by default only Messier/NGC/IC are shown; faint nebula catalogs are hidden',
      () async {
    ...
    test(
      'catalog on/off is applied; multi-membership objects show if any catalog is enabled',
      () async {
        final container = makeContainer([
          _dso('NGC0224', messier: 31),
          _dso('NGC0891'),
    ...
    test(
      'limiting-magnitude filter hides faint DSOs; unknown magnitude is unaffected',
      () async {
        final container = makeContainer([
    ...
    test(
      'settings are persisted and restored across a simulated restart',
      () async {
