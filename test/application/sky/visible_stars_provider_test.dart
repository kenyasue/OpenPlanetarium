...
void main() {
  group('lodPolicy', () {
    ...
    test(
      'effective limiting magnitude is the minimum of LOD limit and user setting',
      () {
  ...
  group('visibleStarsProvider', () {
    test(
      'queries the repository with viewport tiles and the effective limiting magnitude',
      () async {
    ...
    test(
      'changing the magnitude setting re-evaluates the limiting magnitude',
      () async {
        final repo = _RecordingCatalogRepository();
        final container = ProviderContainer(
          overrides: [
  ...
  group('pickObjectAtPoint', () {
    test(
      'selects a star near the click position; outside the threshold returns null',
      () {
        final container = ProviderContainer();
        addTearDown(container.dispose);
    ...
    test(
      'between a nearby star and planet, the brighter (lower magnitude) one wins',
      () {
        final container = ProviderContainer();
        addTearDown(container.dispose);
        final state = container.read(viewportStateProvider);
        ...
        final nearPoint = projection.unproject(
          screenCenter + const Offset(3, 0),
        );
