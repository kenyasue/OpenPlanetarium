...
void main() {
  group('settings persistence', () {
    test(
      'appearance settings are saved and restored in a new container',
      () async {
    ...
    test(
      'constellation settings (including language) are saved and restored',
      () async {
        final repo = InMemorySettingsRepository();
    ...
    test(
      ...
      () async {
        ...
        final restored = await container2.read(
          locationControllerProvider.future,
        );
    ...
    test(
      'legacy JSON with only the bortle key restores with the default limiting magnitude',
      () async {
        final repo = InMemorySettingsRepository();
