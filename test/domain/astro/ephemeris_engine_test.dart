...
void main() {
  ...
  group('Sun position', () {
    test(
      'Meeus example 25.a: apparent Sun position at 1992-10-13 0h TD (±0.05°)',
      () {
  ...
  group('Moon position', () {
    ...
    test(
      'Moon distance stays in the realistic Earth-Moon range (356,000-407,000 km)',
      () {
  ...
  group('Planet positions', () {
    ...
    test(
      'all planets have realistic ecliptic latitude (|β|<9°) (near-ecliptic invariant)',
      () {
        const planets = [
          SolarBodyId.mercury,
          SolarBodyId.venus,
    ...
    test(
      'inner planet elongation from the Sun stays below maximum (Mercury<28.5°, Venus<48°)',
      () {
  ...
  group('moonPhase', () {
    test(
      'illuminated fraction is 0-1 and lunar age is within 0 to one synodic month',
      () {
    ...
    test(
      'near age 0 is new moon (fraction≈0), near age 15 is full moon (fraction≈1)',
      () {
  ...
  group('riseSetTimes', () {
    ...
    test(
      'altitude at rise/set times is close to the rise/set criterion (-0.5667°)',
      () {
        const astro = AstroEngine();
        final sirius = SkyPoint(101.2872, -16.7161);
        final times = engine.riseSetTimes(
